...
import PrmScoreboardVisual from '~/visual/PrmScoreboardVisual';
import PrmBottomBarVisual from '~/visual/PrmBottomBarVisual';
import LckScoreboardVisual from '~/visual/LckScoreboardVisual';
...
export default class IngameScene extends Phaser.Scene {
    ...
    // PRM top bar — instantiated instead of `score` when OverlayConfig.PrmScore.Enabled.
    prmScore: PrmScoreboardVisual | null = null;
    // PRM bottom comparison bar — when OverlayConfig.PrmScore.BottomBar.
    prmBottom: PrmBottomBarVisual | null = null;
    // LCK-style bottom scoreboard — when BottomBar && PrmScore.BottomStyle === 'lck'.
    // Sibling of prmBottom: exactly one of the two is created (selected by BottomStyle).
    lckScoreboard: LckScoreboardVisual | null = null;
    info!: InfoPageVisual;
    // centerGraph is optional under the Vanguard-compatible fork — see Phase 6.
    // GraphVisual instantiation is skipped when GoldGraph.Enabled is not true.
    centerGraph: GraphVisual | null = null;
    ...
    preload() {
        ...
        // The overlay HTML is served at /frontend/index.html, so the document base
        // is /frontend/. The asset paths below are written relative ("frontend/images/..."),
        // which against that base double-paths to /frontend/frontend/... -> 404, and Phaser
        // then renders its green "missing texture" placeholder (the broken top-bar icons).
        // Anchor every relative load at the site root so they resolve to /frontend/...
        // regardless of the page path. Absolute http(s)/cache URLs (DataDragon, team logos)
        // carry a scheme and are left untouched by the loader.
        ...
        //PRM top-bar counter icons (extracted from reference, white-on-transparent)
        this.load.image('prm_dragon', 'frontend/images/prm/dragon.png');
        this.load.image('prm_grub', 'frontend/images/prm/grub.png');
        this.load.image('prm_tower', 'frontend/images/prm/tower.png');
        this.load.image('prm_gold', 'frontend/images/prm/gold.png');
        this.load.image('prm_baron', 'frontend/images/prm/baron.png');
    ...
    create() {
        ...
        const OnNewState = (data: any): void => {
            ...
            // "Scoreboard" master toggle (Component.Ingame.UseCustomScoreboard). The backend gates
            // GameTime serialization SOLELY on that toggle, and GameTime is otherwise always a number
            // (or the -1 game-load sentinel). So an absent GameTime uniquely means the operator turned
            // the scoreboard OFF -> hide the PRM top bar AND the bottom board (both), matching the
            // legacy scoreboard's toggle-off behavior. A -1 / numeric GameTime is kept (each board's
            // own anti-flicker hold still covers transient load / mock<->live frames).
            const scoreboardOff = newState.scoreboard !== undefined && newState.scoreboard.GameTime === undefined;
            if (this.prmScore !== null) {
                if (scoreboardOff)
                    this.prmScore.Stop();
                else
                    this.prmScore.UpdateValues(newState);
            } else {
                this.score.UpdateValues(newState);   // legacy already self-hides on absent GameTime
            }
            // Bottom comparison bar live toggle: feed it only while PrmScore.BottomBar is on AND the
            // scoreboard master toggle is on; otherwise Stop() hides it (and we stop calling
            // UpdateValues so it can't re-Start itself). A config change re-pushes OverlayConfig,
            // updating overlayCfg. Only one of prmBottom / lckScoreboard exists (selected by BottomStyle).
            // Bottom board stays rendered while the Gold Graph is up: the graph draws ABOVE it with an
            // opaque background and wipes in (its mask slide), visually covering the scoreboard along the
            // wipe boundary — and uncovering it when the graph wipes out. (Mutual-exclusive wipe.)
            const bottomOn = !scoreboardOff && this.overlayCfg?.PrmScore?.BottomBar === true;
            if (this.prmBottom !== null) {
                if (bottomOn)
                    this.prmBottom.UpdateValues(newState);
                else
                    this.prmBottom.Stop();
            }
            if (this.lckScoreboard !== null) {
                if (bottomOn)
                    this.lckScoreboard.UpdateValues(newState);
                else
                    this.lckScoreboard.Stop();
            }
            // Legacy objective spawn-timers + power-plays: only drive them in non-PRM mode.
            // Under the PRM top bar they are kept hidden (Stopped at init) and not fed here, so
            // they never re-appear over the bar's edges.
            if (this.prmScore === null) {
            ...
            // Gold Graph: lazily create the center graph the first time gold-diff data arrives. The
            // visual isn't pre-created when GoldGraph.Enabled is false, but the runtime "Gold Graph"
            // toggle (and the Mock feed) can still send data — create it on demand so the toggle works
            // without a config flag / restart. With no data it stays null (or Stops if already shown).
            if (this.centerGraph === null && this.overlayCfg?.GoldGraph &&
                Array.isArray(newState.goldGraph) && newState.goldGraph.length > 0) {
                this.centerGraph = new GraphVisual(this, this.overlayCfg!.GoldGraph, newState.goldGraph);
            }
    ...
    UpdateConfig = (message: OverlayConfigEvent): void => {
        ...
        if (this.overlayCfg === null) {
            ...
            // PRM top bar is opt-in; when enabled it replaces the legacy scoreboard.
            if (message.config.PrmScore?.Enabled === true) {
                this.prmScore = new PrmScoreboardVisual(this, message.config.PrmScore);
                console.log('[LB] PRM scoreboard enabled');
                // The PRM top bar is the complete top-bar UI. Hide the legacy objective spawn-timer
                // and power-play widgets so they don't render over/around the bar's edges (they are
                // created above unconditionally; their UpdateValues calls below are also gated off).
                this.dragonTimer.Stop();
                this.baronTimer.Stop();
                this.baronPowerPlay.Stop();
                this.elderPowerPlay.Stop();
                if (message.config.PrmScore?.BottomBar === true) {
                    if (message.config.PrmScore?.BottomStyle === 'lck') {
                        this.lckScoreboard = new LckScoreboardVisual(this, message.config.PrmScore);
                        console.log('[LB] LCK bottom scoreboard enabled');
                    } else {
                        this.prmBottom = new PrmBottomBarVisual(this, message.config.PrmScore);
                        console.log('[LB] PRM bottom bar enabled');
                    }
                }
            } else {
            ...
            // Phase 6: Gold Graph is opt-in. Without per-player totalGold from
            // /liveclientdata, the graph would render flat zeroes — skip it
            // unless the user has explicitly enabled it (memory reader path).
            if (message.config.GoldGraph?.Enabled === true) {
                this.centerGraph = new GraphVisual(this, message.config.GoldGraph, [new GoldEntry(0, 100), new GoldEntry(40, -100)]);
            } else {
                console.log('[LB] Gold Graph disabled (GoldGraph.Enabled=false); skipping GraphVisual init');
            }
        } else {
            ...
            if (this.prmScore !== null && message.config.PrmScore !== undefined)
                this.prmScore.UpdateConfig(message.config.PrmScore);
            else
                this.score?.UpdateConfig(message.config.Score);
            // Bottom bar is a live toggle in BOTH directions. Lazily create it the first
            // time BottomBar flips on (the icons are preloaded), so re-enabling it via a
            // config change shows it without an overlay reload. The OnNewState gate
            // (BottomBar===true) drives show/hide each tick thereafter.
            if (this.prmScore !== null && this.prmBottom === null && this.lckScoreboard === null && message.config.PrmScore?.BottomBar === true) {
                // Style (PRM vs LCK) is fixed at creation by BottomStyle; switching style at
                // runtime needs a reload (matches how other overlay elements behave).
                if (message.config.PrmScore?.BottomStyle === 'lck') {
                    this.lckScoreboard = new LckScoreboardVisual(this, message.config.PrmScore);
                    console.log('[LB] LCK bottom scoreboard created (runtime enable)');
                } else {
                    this.prmBottom = new PrmBottomBarVisual(this, message.config.PrmScore);
                    console.log('[LB] PRM bottom bar created (runtime enable)');
                }
            } else {
                if (this.prmBottom !== null && message.config.PrmScore !== undefined)
                    this.prmBottom.UpdateConfig(message.config.PrmScore);
                if (this.lckScoreboard !== null && message.config.PrmScore !== undefined)
                    this.lckScoreboard.UpdateConfig(message.config.PrmScore);
            }
            this.info.UpdateConfig(message.config.InfoPage);
            // Only forward config updates to centerGraph if it was initialised.
            // A flag flip from false→true at runtime won't lazily create the
            // graph — that requires a restart, matching how other overlay
            // elements behave under config changes.
