...
export default class GraphVisual extends VisualElement {
    ...
    constructor(scene: IngameScene, cfg: GoldGraphDisplayConfig, data: GoldEntry[]) {
        ...
        if (cfg.Background.UseVideo) {
            ...
            this.BackgroundRect.depth = 10;   // above the bottom scoreboard (max depth 6) so the wipe-in covers it
    ...
    UpdateConfig(cfg: GoldGraphDisplayConfig): void {
        ...
        // Keep the whole graph above the bottom scoreboard. UpdateConfig recreates the chart and can
        // run on a live config push (not just via Start), so re-assert depth here too.
        this.Graph?.setDepth(11);
    ...
    Start(): void {
        ...
        // Render the whole graph ABOVE the bottom scoreboard so its opaque background covers the
        // scoreboard as the mask wipes in (and uncovers it on wipe-out) — that IS the wipe transition.
        this.Graph?.setDepth(11);
        this.Title?.setDepth(12);
