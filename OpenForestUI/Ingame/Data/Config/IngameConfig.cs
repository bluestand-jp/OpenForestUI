...
namespace OpenForestUI.Ingame.Data.Config
{
    public class IngameConfig : JSONConfig
    {
        ...
        // PRM / EMEA Masters style top bar (opt-in). When Enabled, the frontend renders
        // PrmScoreboardVisual instead of the legacy scoreboard. Absent/disabled by default.
        public PrmScoreConfig PrmScore;

        public class PrmScoreConfig
        {
            public bool Enabled;
            public string Font;
            ...
            // Bottom-scoreboard style used when BottomBar is on. "prm" (default/null) keeps the
            // existing PrmBottomBarVisual; "lck" renders the LCK-style scoreboard. Additive to a
            // nested optional object => backward-compatible (old Ingame.json loads with null => "prm").
        ...
        private IngameConfig CreateDefault()
        {
            return new IngameConfig()
            {
                ...
                {
                    // Default OFF (operator opt-in). The graph has a working Vanguard-safe data
                    // source (OCR-exact team gold when locked, item+score estimate otherwise) —
                    // enable per broadcast via the Gold Graph tile / GoldGraph.Enabled.
                    Enabled = false,
        ...
        {
            // Vanguard-compatible fork: per-player totalGold isn't exposed by
            // /liveclientdata/playerlist, so the dynamic gold graph cannot be
            // populated meaningfully. Default off; users with the memory
            // reader enabled can flip this to true.
            public bool Enabled;
            public Vector2 Position;
            public Vector2 Size;
            public BackgroundDisplayConfig Background;
