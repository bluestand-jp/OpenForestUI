...
namespace OpenForestUI.Common.Data.Config
{
    class JSONConfigProvider
    {
        ...
        public void ReadConfig(JSONConfig config)
        {
            ...
            // An empty or corrupt file (e.g. a config write interrupted mid-truncate, leaving 0 bytes)
            // deserializes to null. Treat it like a missing file and restore defaults, rather than
            // throwing an NRE on the dynamic .FileVersion access below (this was a hard startup crash).
            if (readConfig == null)
            {
                Log.Warn($"Config {config.Name} was empty or unreadable. Restoring default config");
                config.RevertToDefault();
                WriteConfig(config);
                return;
            }
        ...
        public void WriteConfig(JSONConfig config)
        {
            ...
            using StreamWriter stream = File.CreateText(Path.Combine(_configPath, $"{config.Name}.json"));
        ...
        public void WriteTeam(JSONConfig config)
        {
            ...
            using StreamWriter stream = File.CreateText(Path.Combine(Path.Combine(_configPath, "Teams"), $"{config.Name}.json"));
            string json = config.GETJson();
            stream.Write(json);
            stream.Close();
