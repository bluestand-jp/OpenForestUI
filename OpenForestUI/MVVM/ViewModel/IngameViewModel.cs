...
namespace OpenForestUI.MVVM.ViewModel
{
    class IngameViewModel : ObservableObject
    {
        ...
        public bool IsActive
        {
            ...
            set
            {
                if (ConfigController.Component.Ingame.IsActive == value) return;
                ConfigController.Component.Ingame.IsActive = value;
                if (value)
                {
                    // Subscribe the overlay to game start. If a game is ALREADY running, GameStart
                    // won't fire again, so start the live overlay tick now so it resumes immediately
                    // (EnterIngame is idempotent; DoTick is guarded against a missing game/process).
                    AppStateController.EnableIngame();
                    if (IngameController.LeagueProcess != null)
                        BroadcastController.Instance.IGController.EnterIngame(this, IngameController.LeagueProcess);
                }
                else
                {
                    AppStateController.DisableIngame();
                }
                OnPropertyChanged();
            }
        }
        ...
        {
            _openCommand = new(o => App.Services.GetRequiredService<INavigationService>().NavigateTo(AppRoute.Ingame));
    ...
    public class PlayersTabViewModel : ObservableObject
    {
        ...
        // (EXP Tab removed: per-player XP has no Vanguard-safe source — partner-only
        //  Livestats or memory reading. See docs/feature-completion/DESIGN.md (b).)
