...
export default class InfoPageVisual extends VisualElement {
    ...
    UpdateValues(newValues: InfoSidePage): void {
        // Guard the whole page shape, not just the Title: a present-but-empty Players
        // list would TypeError on Players[0] below and abort the heartbeat handler
        // mid-update (everything after info.UpdateValues — gold graph, soul point —
        // would silently stop updating).
        if (!newValues || newValues.Title === undefined || newValues.Title === null || newValues.Title == ''
            || !Array.isArray(newValues.Players) || newValues.Players.length === 0) {
            if (this.isActive) {
                this.Stop();
            }
            return;
    ...
    UpdateValues(tabInfo: PlayerInfoTab): void {
        ...
        switch (tabInfo.ExtraInfo[1]) {
            case "gold":
                ...
                min = '';
                // Hide the small raw per-player value (estimated/imprecise); the rounded MainVal (e.g. 4.7k) is kept.
                max = '';
            ...
            case 'cspm':
                ...
                // Hide the small raw per-player value; the rounded MainVal is kept.
                max = '';
