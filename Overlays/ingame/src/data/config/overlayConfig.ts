...
export interface OverlayConfig {
    ...
    GoogleFonts: string[];
    // PRM / EMEA Masters style top bar (opt-in). When Enabled, IngameScene uses
    // PrmScoreboardVisual instead of the legacy ScoreboardVisual. Absent/false in
    // existing configs -> undefined -> legacy scoreboard (unchanged default).
    PrmScore?: PrmScoreConfig;
}

export interface PrmScoreConfig {
    Enabled: boolean;
    // Font family (must be in GoogleFonts or a local/websafe font). Optional.
    Font?: string;
    // Fallback tournament banner text if the state doesn't carry one.
    TournamentName?: string;
    // Bottom comparison bar (Phase 2). Off by default.
    BottomBar?: boolean;
    // Bottom-scoreboard style when BottomBar is on. 'prm' (default/absent) =
    // PrmBottomBarVisual; 'lck' = LckScoreboardVisual.
    BottomStyle?: 'prm' | 'lck';
    // DataDragon version for champion/item/spell icons. If empty the overlay fetches
    // the latest from ddragon versions.json (with a baked-in fallback).
...
export interface GoldGraphDisplayConfig {
    // Vanguard-compatible fork: /liveclientdata/playerlist doesn't expose
    // per-player totalGold, so the live graph has no data to draw. Default
    // false on the backend means this is undefined when not present in JSON;
    // the IngameScene treats undefined as off.
    Enabled?: boolean;
    Position: Vector2;
    Size: Vector2;
